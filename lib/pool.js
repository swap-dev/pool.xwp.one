...
function jobRefresh(loop, callback, caller){
    ...
    getBlockTemplate(function(error, result){
        ...
        if (!currentBlockTemplate || result.height > currentBlockTemplate.height) {
            ...
            log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d (bc to %d miners) (%s)', [result.height, result.difficulty, Object.keys(connectedMiners).length, caller]);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                jobRefresh(false,null,'block found');
            }
        });
    }
